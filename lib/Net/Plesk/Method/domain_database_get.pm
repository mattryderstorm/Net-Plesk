package Net::Plesk::Method::domain_database_get;
...
sub init {
  ...
  $args{domain}      and $filter ||= "<domain-name>$args{domain}</domain-name>";
  $args{database_id} and $filter ||= "<id>$args{database_id}</id>";
  $args{domain_id}   and $filter ||= "<domain-id>$args{domain_id}</domain-id>";
